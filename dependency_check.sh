...
DOCKER_REGISTRY="771070158678.dkr.ecr.us-east-2.amazonaws.com"
REPO_DEV_NAME="demo"
ECR_TAG="demo-project-82"

# Run Syft to generate SBOM and log errors
syft packages docker:$DOCKER_REGISTRY/$REPO_DEV_NAME:$ECR_TAG -o cyclonedx > syft_scanresults 2> syft_error.log

# Debugging: Log BOM_CONTENT_BASE64
BOM_CONTENT_BASE64=$(base64 -w0 syft_scanresults)
echo "BOM_CONTENT_BASE64: $BOM_CONTENT_BASE64" > debug.log

# Create JSON payload for Dependency-Track API
echo '{"project": "'"$Dependency_Track_Project_Identity"'", "bom": "'"$BOM_CONTENT_BASE64"'"}' > json_payload.json

# Debugging: Log JSON_PAYLOAD
JSON_PAYLOAD=$(jq -n --slurpfile json json_payload.json '$json[0]')
